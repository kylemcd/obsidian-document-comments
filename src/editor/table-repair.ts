...
/** Every marker-shaped string in `text`. Only the cheap per-line gate uses this. */
const stripMarkers = (text: string): string => text.replace(ANCHOR_MARKERS, "");

/** The anchor markers the parser recognized, in document order. */
const parsedMarkers = (comments: readonly ParsedComment[]): TextRange[] => {
	return comments
		.flatMap((comment) => [comment.open, comment.close])
		.filter((marker): marker is TextRange => marker !== null)
		.sort((a, b) => a.from - b.from);
};

/**
 * `text` with just these markers taken out. Marker-shaped text the parser skipped
 * — written out inside a code span, say, or hidden by a stray backtick — is
 * ordinary text; stripping it too would delete it, because nothing puts it back.
 */
const withoutMarkers = (text: string, markers: readonly TextRange[]): string => {
	return (
		markers.map((marker, index) => text.slice(markers[index - 1]?.to ?? 0, marker.from)).join("") +
		text.slice(markers[markers.length - 1]?.to ?? 0)
	);
};

/** How many marker characters come out before `position`. */
const strippedBefore = (markers: readonly TextRange[], position: number): number => {
	return markers
		.filter((marker) => marker.from < position)
		.reduce((removed, marker) => removed + (marker.to - marker.from), 0);
};
...
const repairedBlock = (doc: string, block: TextRange, ids: ReadonlySet<string>): string | null => {
	...
	const found = parsedMarkers(comments);
	if (found.length === 0) return null;
	const bare = withoutMarkers(text, found);
	...
	const anchored = comments.filter((comment) => comment.open || comment.close);
	const placements = anchored.map((comment): Placement | null => {
		...
		const from = range.from - strippedBefore(found, range.from);
		const to = range.to - strippedBefore(found, range.to);
...
export const brokenTableAnchors = (doc: string, parsed?: readonly ParsedComment[]): Set<string> => {
	...
	const broken = suspects
		.filter(({ block }) => {
			const text = doc.slice(block.from, block.to);
			return tableCoverage(withoutMarkers(text, parsedMarkers(parseComments(text)))) > tableCoverage(text);
		})
