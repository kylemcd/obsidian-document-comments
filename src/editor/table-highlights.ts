...
/** The cell a source range starts in. `whole` says whether the range also ENDS
 *  there — an anchor spanning two rows has no single cell to paint, but its card
 *  still belongs beside the row it starts on rather than at the top of the table. */
export type AnchorCell = TableCellTarget & { whole: boolean };
...
export const tableHighlightTargets = (doc: string, comments: ParsedComment[]): TableHighlightTarget[] => {
	...
	return comments.flatMap((comment): TableHighlightTarget[] => {
...
export const tableCellForRange = (doc: string, range: TextRange): AnchorCell | null => {
	const lines = sourceLines(doc);
...
const tableCellTarget = (
	...
	const lineIndex = lineIndexAt(lines, range.from);
	const line = lines[lineIndex];
	// The delimiter row renders no cells, so a range starting there has none.
	const table = tables.findIndex(
		({ start, end }) => lineIndex >= start && lineIndex < end && lineIndex !== start + 1,
	);
	const owner = tables[table];
	if (!line || !owner) return null;
	return {
		table,
		// With the delimiter row rendering nothing, body rows shift up by one.
		row: lineIndex === owner.start ? 0 : lineIndex - owner.start - 1,
...
export const tableCellsForRanges = (
	...
	if (ranges.size === 0) return new Map();
	const lines = sourceLines(doc);
	const tables = sourceTables(lines);
	...
	return new Map(
		Array.from(ranges).flatMap(([key, range]): Array<[string, HTMLElement]> => {
...
class TableHighlights {
	...
	commentAtPoint(x: number, y: number): string | null {
		...
		const hit = Array.from(this.painted).find(([, ranges]) =>
			ranges.some((range) => Array.from(range.getClientRects()).some(covers)),
		);
		return hit?.[0] ?? null;
	}
