...
- Added an **Allow empty comments** setting. An empty comment now saves a highlight without a comment card or sidebar entry. Run **Add comment** on the same text to convert or remove the highlight ([#52](https://github.com/kylemcd/obsidian-document-comments/issues/52)).
