...
describe("tableHighlightTargets", () => {
	...
	test.each([
		["outside any table", "Some prose."],
		["on the delimiter row", "| --- | --- |"],
	])("finds no cell for a range starting %s", (_label, text) => {
		const doc = ["Some prose.", "", "| Day | Task |", "| --- | --- |", "| Monday | spec |"].join("\n");
		const from = doc.indexOf(text) + 2;

		expect(tableCellForRange(doc, { from, to: from + 1 })).toBeNull();
	});
