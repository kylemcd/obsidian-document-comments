...
describe("brokenTableAnchors", () => {
	...
	test("still finds a broken anchor beside an orphaned comment", () => {
		// An orphan keeps its body after its anchored text is deleted, so it has no
		// markers in the block at all. It has nothing to move and must not stop the
		// anchors that do from being found.
		const doc = [
			"| Day | Task |",
			"| --- | --- |",
			"<!--c:aa11-->| Monday | spec |<!--/c:aa11-->",
			"| Tuesday | review |",
			body("aa11", "x"),
			body("bb22", "deleted text"),
		].join("\n");

		expect([...brokenTableAnchors(doc)]).toEqual(["aa11"]);
	});

	test("offers no repair while a comment in the table has lost one marker", () => {
		// Stripping a lone marker would delete it with nothing to put back.
		const doc = [
			"| Day | Task |",
			"| --- | --- |",
			"<!--c:aa11-->| Monday | spec |<!--/c:aa11-->",
			"| Tuesday | <!--c:bb22-->review |",
			body("aa11", "x"),
			body("bb22", "review"),
		].join("\n");
	...
	test("finds an anchor that filled the blank line above a table", () => {
		// A triple-click selects a paragraph AND its newline, so the close marker
		// used to land on the blank line a table needs above it.
		const doc = [
			"<!--c:aa11-->Intro text",
			"<!--/c:aa11-->",
			"| Day | Task |",
			"| --- | --- |",
			"| Monday | spec |",
			body("aa11", "Intro text"),
		].join("\n");

		expect([...brokenTableAnchors(doc)]).toEqual(["aa11"]);
	});

	test("does not flag a code comment's own-line markers", () => {
		const doc = [
			"<!--c:cc11-->",
			"```js",
			"const a = 1;",
			...
			"| Day | Task |",
			"| --- | --- |",
			'<!--co:cc11 by:me at:2026-01-01T00:00:00.000Z status:open quote:"const a = 1;" line:0',
			"me: hi",
			"-->",
		].join("\n");
	...
	test("is empty for an empty document", () => {
		expect([...brokenTableAnchors("")]).toEqual([]);
		expect(computeRepairTableAnchors("").isOk()).toBe(true);
	});
...
describe("computeRepairTableAnchors", () => {
	...
	test("repairs past an orphaned comment and leaves its body alone", () => {
		const doc = [
			"| Day | Task |",
			"| --- | --- |",
			"<!--c:aa11-->| Monday | spec |<!--/c:aa11-->",
		...
		expect(out).toContain("| <!--c:aa11-->Monday | spec<!--/c:aa11--> |");
		expect(out).toContain(body("bb22", "deleted text"));
	});

	test("rewrites only the table's own block, not the paragraphs around it", () => {
		const table = [
			"| Day | Task |",
			"| --- | --- |",
			"<!--c:aa11-->| Monday | spec |<!--/c:aa11-->",
		...
		expect(result.isOk() && result.value.map(({ from, to }) => [from, to])).toEqual([
			[doc.indexOf(table), doc.indexOf(table) + table.length],
		]);
	});

	test("moves a marker off the blank line above a table and restores the table", () => {
		const doc = [
			"<!--c:aa11-->Intro text",
			"<!--/c:aa11-->",
			"| Day | Task |",
	...
	test("keeps a literal marker inside a code span when repairing its table", () => {
		...
		expect(out).toContain("| `<!--c:ID-->` | opens a comment |");
		expect(out).toContain("| <!--c:aa11-->Monday | spec<!--/c:aa11--> |");
	});

	test("keeps markers a stray backtick hides from the parser when repairing their table", () => {
		const doc = [
			"| Day | Note | Code |",
			"| --- | --- | --- |",
			"<!--c:aa11-->| Monday | spec | x |<!--/c:aa11-->",
			"| Tuesday | it`s <!--c:bb22-->good<!--/c:bb22--> | `code` |",
			body("aa11", "x"),
			body("bb22", "good"),
		].join("\n");
		...
		expect(out).toContain("| Tuesday | it`s <!--c:bb22-->good<!--/c:bb22--> | `code` |");
		expect(out).toContain("| <!--c:aa11-->Monday | spec | x<!--/c:aa11--> |");
	});
